...
public class Roll3DView extends View {
    ...
    public void toNext(int rotateDegree) {
        if (rolling)
            return;

        if (rollMode == RollMode.RollInTurn) {
            valueAnimator = ValueAnimator.ofFloat(rotateDegree, 90 + (partNumber - 1) * 30);
        } else if (rollMode == RollMode.Jalousie) {
            valueAnimator = ValueAnimator.ofFloat(rotateDegree, 180);
        } else {
            valueAnimator = ValueAnimator.ofFloat(rotateDegree, 90);
        }
        rolling = true;
        valueAnimator.setDuration(rollDuration);
        valueAnimator.addUpdateListener(updateListener);
    ...
    public void toPre(int rotateDegree) {
        if (rolling)
            return;
        int startRotate = 0;
        ...
        startRotate=startRotate-rotateDegree;
        //rotateDegree == 0 说明curr在当前显示
        //设置角度为90或180 nextIndex和currIndex preIndex轮转互换,使next显示到当前的图片，然后完成翻转
        //可以通俗的理解为  先倒过来 再翻过去  只不过倒过来之前把图片也互换了 所以看不出来而已
        rollIndex(true);
